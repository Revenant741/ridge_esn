...
def reset_weight_in():
  weight_in = []
  with open('ga_data/bias_weight_in1.dat','rb') as f:
    d2 = pickle.load(f)
    weight_in = d2
...
def reset_weight_res(i):
  num = i+1
  weight_res = []
  with open('ga_data/bias_weight1.dat','rb') as f:
    d2 = pickle.load(f)
    weight_res = d2
  np.set_printoptions(threshold=np.inf)
  weight_res = weight_res[-num]
  return weight_res
...
if __name__ == '__main__':
  for j in range(2):
    ...
    plt.xlim(0,0.7)
    plt.ylim(0,0.7)
    plt.savefig('img/mutial_info_evolved'+str(j)+'.png')
    plt.savefig('img/mutial_info_evolved'+str(j)+'.pdf')

...
class Binde_ESN_Model:
  ...
  def reset_weight_in(self):
      ...
      bind = np.concatenate([np.ones((self.size_in,self.size_in)), np.zeros((self.size_in,self.size_in))])
      weight_in *= bind
      return weight_in
  ...
  def reset_weight_res(self):
      ...
      adjency = np.array([random.randint(0, 1) for _ in range(self.size_middle**2)])
  ...
  def reset_weight_out(self):
      ...
      bind = np.concatenate([np.zeros((self.size_out,self.size_in)), np.ones((self.size_out,self.size_in))],1)
      weight_out *= bind
      return weight_out
  ...
  def forward(self, u, weight_in, weight_res, b_in, b_res):
    ...
    sp_pre = torch.tanh(torch.matmul(self.sp_weight_out,self.state))
    tp_pre = torch.tanh(torch.matmul(self.tp_weight_out,self.state))
  ...
  #クロスエントロピー誤差
  def cross_entropy_error(self, out_put, ans,data_size):
    #delta = 1e-4\
    loss = -np.sum(ans*np.log(out_put))/data_size
    return loss
